...
@Component({
  selector: 'dialog-import-nam-hoc',
  standalone: true,
...
export class DialogImportNamHocComponent extends ComponentBaseAbstract {
  $formItem: FormType[] = [
    FILE_CONTROL({
      controlName: 'file',
      required: true,
  ...
  constructor(
    ...
    private readonly dialogRef: MatDialogRef<DialogImportNamHocComponent>,
  ...
  downloadTemplate(): void {
    this.namHocService.downloadTemplate().subscribe({
      next: (response: any) => {
        const blob = response?.data ?? response;
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = 'template_import_nam_hoc.xlsx';
        a.click();
  ...
  onSubmit(): void {
    ...
    this.namHocService.import(formData).subscribe({
  ...
  private downloadErrorFile(
    ...
    this.namHocService.downloadImportErrorFile(errorFileToken).subscribe({
      next: ({ body, headers }) => {
        if (!body) {
