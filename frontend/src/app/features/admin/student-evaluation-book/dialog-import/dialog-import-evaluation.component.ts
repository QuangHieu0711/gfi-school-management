...
export class DialogImportEvaluationComponent extends ComponentBaseAbstract {
  ...
  constructor(
    ...
    public data: { classroomId: number; subjectId: number; semesterId: number; classroomName: string; subjectName: string; semesterName: string }
  ) {
    super(injector);
  ...
  downloadTemplate(): void {
    this.evaluationService.exportTemplate(this.data.classroomId, this.data.subjectId, this.data.semesterId).subscribe({
      next: (blob: Blob) => {
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `mau_import_danh_gia_${this.data.classroomName}_${this.data.subjectName}.xlsx`;
        a.click();
  ...
  onSubmit(): void {
    ...
    this.evaluationService.importExcel(file, this.data.classroomId, this.data.subjectId, this.data.semesterId).subscribe({
      next: ({ data, userMessage }) => {
        const successCount = data?.successCount ?? 0;
        const failedCount = data?.failedCount ?? 0;
        const message = userMessage || `Import hoàn tất: ${successCount} bản ghi thành công, ${failedCount} bản ghi lỗi`;
